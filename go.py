...
# worker -> windows : 0
parser.add_argument('--data', default='/home/chenjx/data/FGNET/images', type=str, help='datasets path')
parser.add_argument('--dim', default=64, type=int, help='embedding size')
parser.add_argument('--epochs', default=100, type=int)
parser.add_argument('-j', '--workers', default=2, type=int)
parser.add_argument('-M', default = 20, type = int , help = 'MPerClassSampler')
parser.add_argument('--batch_size', default=20, type=int)
parser.add_argument('--eval_batch_size', default=120, type=int)
parser.add_argument('--lr', default=1e-4, type=float)
parser.add_argument('--gpu', default=None, type=int)
parser.add_argument('--only_dml' , default = False, type = bool)
...
parser.add_argument('--vartheta', '-vt', default = 0.05,type = float)
parser.add_argument('--varepsilon', '-ve', default=0.05, type = float)
...
def train(Backbone, Rel_Net, Abs_Net, loss_funcA, loss_funcB, pop_train_loader, abs_train_loader , aug_transform, optimizer, epoch):
    start_time = time.time()
    Backbone.train()
    freeze_BN(Backbone)
    lossAList, lossRList = [] , []
    if not args.only_dml:
    ...
    if not args.only_dml:
        writer.add_scalar('loss/lossA', lossA, epoch + 1)
        writer.add_scalar('loss/lossR', lossR, epoch + 1)
        writer.add_scalar('loss/WeightedLoss', loss_total, epoch + 1)
        print(f" Epoch:{epoch + 1} -> lossA : {lossA} , lossR : {lossR}, loss_total : {loss_total} , time_cost : {end_time - start_time : .2f}s")
    else:
        writer.add_scalar('DML/lossA', lossA, epoch + 1)
        print(f" Epoch:{epoch + 1} -> lossA : {lossA} , time_cost = {end_time - start_time:.2f}s")
